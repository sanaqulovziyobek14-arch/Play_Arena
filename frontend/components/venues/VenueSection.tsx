...
export default function VenueSection() {
  ...
  useEffect(() => {
    venuesAPI.getAll()
      .then(res => setVenues(res.results.slice(0, 8)))
      .catch(console.error)
      ...
      <div style={{ maxWidth: "1440px", margin: "0 auto", padding: "0 32px" }}>
        ...
        {loading && (
          <div style={{ display: "grid", gridTemplateColumns: "repeat(4,1fr)", gap: "14px" }}>
            {[...Array(4)].map((_,i) => (
              <div key={i} style={{ height: "220px", borderRadius: "14px", background: "rgba(255,255,255,0.04)", animation: "shimmer 1.5s infinite" }} />
            ))}
          </div>
        )}
        ...
        {!loading && venues.length > 0 && (
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(240px,1fr))", gap: "14px" }}>
            {venues.map(venue => {
              ...
              const rating = typeof venue.rating === "number" && venue.rating > 0 ? venue.rating : null;
              return (
                  ...
                  onMouseLeave={e => {
                    ...
                    <div style={{
                      ...
                      <button onClick={e => toggleFav(venue.id, e)} style={{
                        ...
                        display: "flex", alignItems: "center", justifyContent: "center",
                        cursor: "pointer", fontSize: "13px", zIndex: 2,
                      ...
                      {venue.sport_name && (
                        <span style={{
                          ...
                          padding: "3px 9px", borderRadius: "6px", zIndex: 2,
                    ...
                    <div style={{ padding: "12px 14px" }}>
                      <div style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between", gap: "8px", marginBottom: "6px" }}>
                        <h3 style={{ fontSize: "13px", fontWeight: 700, color: "#fff", lineHeight: 1.3 }}>
                          {venue.name}
                        </h3>
                        {rating !== null && (
                      ...
                      <div style={{ fontSize: "14px", fontWeight: 800, color: "#22c55e" }}>
                        {Number(venue.price).toLocaleString()} <span style={{ fontSize: "10px", color: "rgba(255,255,255,0.3)", fontWeight: 400 }}>so&apos;m/soat</span>
                      </div>
                    </div>
                  </div>
                </Link>
